...
export default function DashboardPage() {
  const stats = [
    { label: 'TOTAL VISITORS', value: '3,291,922', icon: <Users size={42} />, bg: 'bg-teal', link: 'View Detail' },
    { label: 'BOUNCE RATE', value: '20.44%', icon: <TrendingUp size={42} />, bg: 'bg-blue', link: 'View Detail' },
    { label: 'UNIQUE VISITORS', value: '1,291,922', icon: <Users size={42} />, bg: 'bg-orange', link: 'View Detail' },
    { label: 'AVG TIME ON SITE', value: '00:12:23', icon: <Clock size={42} />, bg: 'bg-red', link: 'View Detail' },
  ]
          ...
          <div className="panel-body">
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                  <th className="text-left py-2 font-semibold" style={{ color: 'var(--foreground)' }}>Source</th>
                  <th className="text-right py-2 font-semibold" style={{ color: 'var(--foreground)' }}>Total</th>
                  <th className="text-right py-2 font-semibold" style={{ color: 'var(--foreground)' }}>Trend</th>
                </tr>
              </thead>
              <tbody>
                {sources.map((s) => (
                  <tr key={s.name} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td className="py-2 text-right" style={{ color: 'var(--foreground)' }}>{s.percent}%</td>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="p-3 rounded" style={{ background: 'var(--background-secondary)' }}>
                <div className="text-xs font-semibold" style={{ color: 'var(--foreground-muted)' }}>Total Page Views</div>
                <div className="text-lg font-semibold" style={{ color: 'var(--foreground)' }}>1,230,000</div>
              </div>
              <div className="p-3 rounded" style={{ background: 'var(--background-secondary)' }}>
                <div className="text-xs font-semibold" style={{ color: 'var(--foreground-muted)' }}>Avg Time On Site</div>
                <div className="text-lg font-semibold" style={{ color: 'var(--foreground)' }}>00:03:45</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="panel">
        ...
        <div className="panel-body">
          <div className="space-y-2">
            {[
              { text: 'Donec vehiculum pretium nisl, id lacinia nisl condimentum id', done: true, color: 'bg-teal' },
              { text: 'Duis a ullamcorper massa', done: false, color: 'bg-blue' },
              { text: 'Phasellus bibendum, odio nec vestibulum ullamcorper', done: false, color: 'bg-orange' },
              { text: 'Duis pharetra mi sit amet dictum cursus', done: false, color: 'bg-red' },
            ].map((todo, i) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded" style={{ background: 'var(--background-secondary)' }}>
                <span className={`w-1.5 h-8 rounded ${todo.color}`}></span>
                <input type="checkbox" checked={todo.done} readOnly className="w-4 h-4" />
                <span className={`flex-1 text-sm ${todo.done ? 'line-through opacity-50' : ''}`} style={{ color: 'var(--foreground)' }}>
                  {todo.text}
                </span>
              </div>
            ))}
